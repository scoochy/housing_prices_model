...
disposable_income <- disposable_income %>% 
  ...
  filter(year %in% 1987:2019) %>% 
  select(year, disposable_income)

money_supply <- money_supply %>% 
  mutate(date = ymd(DATE), supply = MANMM101GBM189S) %>% 
  filter(date > as.Date("1987-01-01") & date < as.Date("2019-12-31")) %>% 
...
bubble <- tibble(year = 1987:2019, bubble = if_else(year %in% c(1987, 1988, 1989, 2002:2007), TRUE, FALSE))
...
df <- tibble(year = mortgage_ratio_wages$year, disposable_income = disposable_income$disposable_income,
             ...
             interest_rates = interest_rates_filled$average_interest_rate, gdp = gdp_ph$`Gross domestic product (Average) per head, CVM market prices: SA`,
             ...
             mortgage_ratio_wages = mortgage_ratio_wages$average_mort_ratio_wages, bubble = bubble$bubble)

write_csv(df, "clean data/bubble_data.csv")
